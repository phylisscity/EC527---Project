 ...
 *
 * radix_sort_omp.c - OpenMP parallelized LSD radix sort
 *
 * Compile: gcc -O1 -fopenmp radix_sort_omp.c -lrt -o radix_sort_omp
 *
 * Optimization over serial baseline: OpenMP multicore parallelism on the count step.
 * Each thread independently counts its own chunk of the input into a private
 * count array, then a serial reduction combines all thread counts into one.
 * The scatter step remains serial — threads would race to write the same
 * output positions if parallelized without heavy synchronization.
 *
 * Algorithm: LSD (least significant digit) radix sort
 *   - 8-bit radix: 256 buckets, 4 passes for 32-bit unsigned integers
 *   - Each pass: parallel count -> reduction -> scan (prefix sum) -> scatter
...
#define NUM_THREADS 8           // set to number of physical cores on signals53
...
// ---------- OpenMP radix sort pass ----------

/*
 * radix_sort_pass: one pass of LSD radix sort with parallelized count step
 *
 * Steps:
 *   1. Count (parallel): each thread counts its own chunk of input
 *      into a private row of thread_counts[thread_id][bucket]
 *   2. Reduction: combine all thread count arrays into one global count[]
 *   3. Scan: prefix sum over count[] -> starting output position per bucket
 *   4. Scatter: place each element at its correct output position (serial)
 */
void radix_sort_pass(unsigned int *in, unsigned int *out, long int n, int shift)
{
    ...
    // per-thread count arrays - each thread writes to its own row, no races
    long int thread_counts[NUM_THREADS][RADIX];
    memset(thread_counts, 0, sizeof(thread_counts));

    // step 1: parallel count - each thread handles its own chunk
    #pragma omp parallel num_threads(NUM_THREADS)
    {
        int tid = omp_get_thread_num();
        int nthreads = omp_get_num_threads();

        long int chunk = (n + nthreads - 1) / nthreads;
        long int start = tid * chunk;
        long int end   = start + chunk;
        if (end > n) end = n;

        for (long int i = start; i < end; i++)
            thread_counts[tid][(in[i] >> shift) & 0xFF]++;
    }

    // step 2: reduction - combine thread counts into global count[]
    memset(count, 0, sizeof(count));
    for (int t = 0; t < NUM_THREADS; t++)
        for (int b = 0; b < RADIX; b++)
            count[b] += thread_counts[t][b];

    // step 3: exclusive prefix sum - prefix[b] = starting output index for bucket b
    prefix[0] = 0;
    for (int b = 1; b < RADIX; b++)
        prefix[b] = prefix[b-1] + count[b-1];

    // step 4: scatter - serial to avoid write races on output positions
    for (long int i = 0; i < n; i++) {
...
int main(int argc, char *argv[])
{
    ...
    printf("EC527 Final Project - OpenMP Radix Sort\n");
    printf("LSD radix sort: %d-bit radix, %d passes, %d threads\n\n",
